...
public:
    ...
    Mode& compressCTU(CUData& ctu, Frame& frame, const CUGeom& cuGeom, const Entropy& initialContext, uint8_t* start_depths);
...
protected:
    ...
    void compressIntraCU(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp, uint8_t* start_depths);
    ...
    /* KDC2 : Skip the depth level if less than start_depth */
    inline void skipBestMode(Mode& mode, uint32_t depth, uint8_t startdepth)
    {
        ModeDepth& md = m_modeDepth[depth];
        if (md.bestMode)
        {
