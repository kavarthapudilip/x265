...
void FrameEncoder::processRowEncoder(int intRow, ThreadLocalData& tld)
{
    ...
    while (curRow.completed < numCols)
    {
		...
		//KDC2 : create array of startdepths
		uint8_t* start_depths = (uint8_t*)malloc   ( sizeof(uint8_t)*(numPartitions) );
		...
		if (mrMode == 2)
		{
        	...
        	//KDC2
        	seek_in = fseek(m_top->m_mrDataFile_id, seek_pos, SEEK_SET);
        	if (seek_in) fputs("Seeking error", stderr);
        	size_read = fread(start_depths, sizeof(uint8_t), numPartitions, m_top->m_mrDataFile_id);
        	if (size_read != numPartitions) fputs("Reading error", stderr);
		}

        // Does all the CU analysis, returns best top level mode decision
        Mode& best = tld.analysis.compressCTU(*ctu, *m_frame, m_cuGeoms[m_ctuGeomMap[cuAddr]], rowCoder, start_depths);
		...
		if (mrMode == 1)
		{
			...
			//KDC2 : Creating start depth arrays based on the best depth arrays


        	for (uint32_t i = 0;i<numPartitions;i = i + 16)
        	{
        		if (ctu->m_cuDepth[i] == 3 && (i%256) < 64)
        		{
        			memset ((start_depths + 64), 1, 192);
        			memset (start_depths, 2, 64);
        		}
        		if (ctu->m_cuDepth[i] == 3 && (i%256) >= 64 && (i%256) < 128)
        		{
        		    if(*start_depths != 1)
        		    {
        		    	memset ((start_depths), 1, 64);
        		    	memset ((start_depths + 128), 1, 128);
        		    }
        		    memset ((start_depths + 64), 2, 64);
        		}
        		if (ctu->m_cuDepth[i] == 3 && (i%256) >= 128 && (i%256) < 192)
        		{
        		    if(*start_depths != 1)
        		    {
        		    	memset ((start_depths), 1, 128);
        		    	memset ((start_depths + 192), 1, 64);
        		    }
        		    memset ((start_depths + 128), 2, 64);
        		}
        		if (ctu->m_cuDepth[i] == 3 && (i%256) >= 192 && (i%256) < 256)
        		{
        		    if(*start_depths != 1)
        		    {
        		    	memset ((start_depths), 1, 192);
        		    }
        		    memset ((start_depths + 192), 2, 64);
        		    //break;
        		}
        	}
        	...
        	seek_in = fseek(m_top->m_mrDataFile_id, seek_pos, SEEK_SET);
        	if (seek_in) fputs("Seeking error", stderr);
        	fwrite(start_depths, sizeof(uint8_t), numPartitions, m_top->m_mrDataFile_id);
        	if (size_w != numPartitions) fputs("Reading error", stderr);

		}

		//KDC2 : freeing the memory for startdepths
		free (start_depths);
		start_depths = NULL;
