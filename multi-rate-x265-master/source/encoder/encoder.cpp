...
void Encoder::create()
{
	...
	// multi-rate mode //KDC2 : adding the new file
	if (m_param->mrMode == 1)
		{m_mrDataFile = fopen("analysisData.bin", "wb"); // write, binary
		m_mrDataFile_id = fopen("analysisData_id.bin", "wb");}
	else if (m_param->mrMode == 2)
		{m_mrDataFile = fopen("analysisData.bin", "rb"); // read, binary
		m_mrDataFile_id = fopen("analysisData_id.bin", "rb");}
