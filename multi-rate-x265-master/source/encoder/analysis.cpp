...
Mode& Analysis::compressCTU(CUData& ctu, Frame& frame, const CUGeom& cuGeom, const Entropy& initialContext, uint8_t* start_depths)
{
    m_slice = ctu.m_slice;
    m_frame = &frame;
    ...
    if (m_slice->m_sliceType == I_SLICE)
    {
        ...
        compressIntraCU(ctu, cuGeom, qp, start_depths);
    }
    else
    {
...
void Analysis::compressIntraCU(const CUData& parentCTU, const CUGeom& cuGeom, int32_t qp, uint8_t* start_depths)
{
    uint32_t depth = cuGeom.depth;

    //KDC2 : Adding a bool depthcond which should skip checking of rdcost and split the CU
    uint8_t startDepth = start_depths[cuGeom.absPartIdx];
    bool depthcond = ((depth >= startDepth) && (m_param->mrMode == 2));
    ...
    else if (cuGeom.log2CUSize != MAX_LOG2_CU_SIZE && mightNotSplit)
    {
        ...
        //KDC2 : Skip the depth if less than startdepth
        if (depthcond)
        {
        	FixIntraRd(md.pred[PRED_INTRA], cuGeom, SIZE_2Nx2N);
        	skipBestMode(md.pred[PRED_INTRA], depth, startDepth);
        }
        else
        {
        ...
        if (cuGeom.log2CUSize == 3 && m_slice->m_sps->quadtreeTULog2MinSize < 3)
        {
            ...
            //KDC2 : Same condition as above applying here too
            if (depthcond)
            {
            	FixIntraRd(md.pred[PRED_INTRA], cuGeom, SIZE_2Nx2N);
            	skipBestMode(md.pred[PRED_INTRA], depth, startDepth);
    ...
    if (mightSplit)
    {
        ...
        for (uint32_t subPartIdx = 0; subPartIdx < 4; subPartIdx++)
        {
            ...
            if (childGeom.flags & CUGeom::PRESENT)
            {
                ...
                compressIntraCU(parentCTU, childGeom, nextQP, start_depths);
